...
''')
c.execute('''
CREATE TABLE IF NOT EXISTS citizen_central_service_record (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    date TEXT NOT NULL,
    name TEXT NOT NULL,
    mobile TEXT NOT NULL,
...
# Feedback page content
    ...
    """, unsafe_allow_html=True)
    counter_no = st.selectbox("", ["Counter_1", "Counter_2", "Counter_3", "Counter_4"])

    # Dropdown for overall rating on counter service
    st.markdown("<p style='font-size: 20px;'>Counter No Service Rating:</p>", unsafe_allow_html=True)
...
""", unsafe_allow_html=True)
    ...
    if st.session_state.get("authenticated", False):
        ...
        if st.button("Open Dashboard"):
            script_path = r"E:\SIH FINAL\codes\dash.py"
